...
(function wireLiveMonitor(){
  ...
  function renderDetections(recent) {
    ...
    const html = sorted.slice(0, 30).map(d => {
      ...
      if (isNew) {
        ...
        // Accumulate attack type counts for breakdown chart
        const t = d.predicted || 'Unknown';
        sessionAttacks[t] = (sessionAttacks[t] || 0) + 1;
        // Track newest critical for banner
      ...
      return `
        <div class="mon-det-row${isNew ? ' mon-det-new' : ''}"
             style="padding:10px 14px;border-bottom:1px solid #1e3557;display:flex;align-items:center;gap:10px;font-size:12.5px;flex-wrap:wrap">
          <span style="color:#4a6080;font-family:monospace;font-size:11px;min-width:68px;flex-shrink:0">${ts}</span>
    ...
    // Show critical banner for newest unseen critical attack
    if (newCritical) showCriticalBanner(newCritical);
  }

  // ── Attack Breakdown Chart ──────────────────────────────────────────
  const TYPE_COLORS = {
    'DDoS':'#ef4444', 'DoS':'#f97316', 'Brute Force':'#f59e0b',
    'Port Scan':'#22b8cf', 'Bot':'#a78bfa', 'XSS':'#ec4899',
    'Web Attack':'#10b981', 'ICMP Flood':'#fb923c', 'ICMP Sweep':'#38bdf8',
  };
  ...
  function renderAttackBreakdown() {
    const wrap  = document.getElementById('monBreakdownWrap');
    const bars  = document.getElementById('monBreakdownBars');
    const total = document.getElementById('monBreakdownTotal');
    if (!wrap || !bars) return;

    const entries = Object.entries(sessionAttacks).sort((a, b) => b[1] - a[1]);
    if (!entries.length) { wrap.style.display = 'none'; return; }

    wrap.style.display = '';
    const max = entries[0][1];
    const sum = entries.reduce((a, [, c]) => a + c, 0);
    if (total) total.textContent = `${sum} total attack${sum !== 1 ? 's' : ''} this session`;

    bars.innerHTML = entries.map(([type, count]) => {
      const pct   = Math.max(4, (count / max) * 100);
      const color = typeColor(type);
      return `
        <div class="mon-bdown-row">
          <span class="mon-bdown-label" title="${type}">${type}</span>
          <div class="mon-bdown-track">
            <div class="mon-bdown-fill" style="width:${pct}%;background:${color}"></div>
          </div>
          <span class="mon-bdown-count">${count}</span>
        </div>`;
    }).join('');
  }

  // ── Critical Attack Banner ──────────────────────────────────────────
  const critBanner     = document.getElementById('criticalBanner');
  const critDetail     = document.getElementById('critBannerDetail');
  const critBlockBtn   = document.getElementById('critBannerBlock');
  const critCloseBtn   = document.getElementById('critBannerClose');
  let   critHideTimer  = null;

  if (critCloseBtn) critCloseBtn.addEventListener('click', hideCriticalBanner);
  if (critBlockBtn) {
    critBlockBtn.addEventListener('click', async () => {
      if (!lastCriticalIp) return;
      critBlockBtn.disabled = true;
      critBlockBtn.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i> Blocking…';
      try {
        ...
        setTimeout(hideCriticalBanner, 1500);
      } catch (err) {
        critBlockBtn.disabled = false;
        critBlockBtn.innerHTML = '<i class="fa-solid fa-ban"></i> Block Source';
        showToast('Block failed: ' + (err.message || 'error'), true);
      }
    });
  }

  function showCriticalBanner(d) {
    if (!critBanner || !critDetail) return;
    lastCriticalIp = d.src_ip || null;
    const src = d.src_ip ? `${d.src_ip}${d.src_port ? ':' + d.src_port : ''}` : '—';
    const dst = d.dst_ip ? `${d.dst_ip}` : '—';
    critDetail.textContent = `${d.predicted} · from ${src} → ${dst} · confidence ${((d.confidence||0)*100).toFixed(1)}%`;
    if (critBlockBtn) {
      const alreadyBlocked = lastCriticalIp && blockedIps.has(lastCriticalIp);
      critBlockBtn.disabled = alreadyBlocked;
      critBlockBtn.innerHTML = alreadyBlocked
        ? '<i class="fa-solid fa-check"></i> Blocked'
        : '<i class="fa-solid fa-ban"></i> Block Source';
    }
    critBanner.style.display = '';
    clearTimeout(critHideTimer);
    critHideTimer = setTimeout(hideCriticalBanner, 12000);
  }

  function hideCriticalBanner() {
    if (critBanner) critBanner.style.display = 'none';
    clearTimeout(critHideTimer);
  }

  // ── Block button event delegation ───────────────────────────────────
  detectionsList.addEventListener('click', async (e) => {
    const btn = e.target.closest('.mon-block-btn');
