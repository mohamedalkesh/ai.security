...
function render(){
  ...
  body.innerHTML = pageRows.map((a, i) => {
    ...
    // Threat Intelligence reputation badge
    const score = a.abuse_score ?? a.abuseScore;
    const reports = a.abuse_reports ?? a.abuseReports;
    const abuseCountry = a.abuse_country ?? a.abuseCountry ?? '';
    let repBadge = '';
    if (score != null) {
      const col = score >= 75 ? '#ff6b6b' : score >= 30 ? '#ffa94d' : '#51cf66';
      const tip = `AbuseIPDB: score ${score}/100 · ${reports || 0} reports${abuseCountry ? ' · ' + abuseCountry : ''}`;
      repBadge = `<span title="${tip}" style="margin-left:4px;font-size:10px;font-weight:700;
        padding:2px 6px;border-radius:4px;border:1px solid ${col}33;
        background:${col}18;color:${col};cursor:default">
        <i class="fa-solid fa-shield-halved"></i> ${score}</span>`;
    }
    return `
    <tr style="animation-delay:${i*40}ms" data-id="${a.id}">
      ...
      <td class="cell-ip">${flag ? flag + ' ' : ''}${a.source_ip || '—'}${repBadge}</td>
